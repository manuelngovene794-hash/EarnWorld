...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const [mode, setMode] = useState<'login' | 'register' | 'phone'>('register');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [name, setName] = useState('');
  const [country, setCountry] = useState('MZ'); // Default to Mozambique
  ...
  const handleSendSms = () => {
    ...
    setErrorMsg('');
    setSmsSent(true);
    setSmsCode('123456'); // Simulated code for testing ease
  };
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-slate-900 border border-amber-500/30 p-6 sm:p-7 shadow-2xl my-8">
        ...
        <div className="text-center space-y-1 mb-6">
          ...
          <p className="text-xs text-slate-400">
            {mode === 'login' ? 'Aceda à sua conta e saldo de recompensas' : 'Crie a sua conta gratuita e receba bónus de boas-vindas'}
          </p>
        </div>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-3.5">
            {mode === 'register' && (
              <>
                ...
                {/* Country dropdown with Mozambique prominently */}
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-400">País de Residência</label>
                  <div className="relative">
            ...
            >
              {loading ? 'A processar...' : mode === 'login' ? 'Entrar na Conta' : 'Criar Conta Gratuita (+150 PTS)'}
            </button>
          </form>
        ) : (
          /* Phone / SMS Form */
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input
                  type="tel"
                  required
                  placeholder={country === 'MZ' ? '84XXXXXXX ou 85XXXXXXX' : 'Número com indicativo'}
                  value={phoneNumber}
            ...
            {!smsSent ? (
              ...
              <div className="space-y-2 p-3 rounded-xl bg-slate-950 border border-amber-500/30">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-300">Código de Verificação SMS:</span>
        ...
        {/* Demo Fast Access Button for Mozambique */}
        <div className="mt-5 pt-4 border-t border-slate-800 text-center">
          ...
          >
            🇲🇿 Testar Imediatamente como Manuel Ngovene (Super Admin Moçambique)
          </button>
        </div>
